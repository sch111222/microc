module ConsoleApplication7.Parser
...
let forStatement              = nonTerminal<ForStatement>()
...
statement.AddProduction(forStatement)       .SetReduceFunction(fun  a -> Ast.ForStatement a)
...
forStatement.AddProduction(forkeyword,openParen,expression,comma,statement,closeParen,statement)
   .SetReduceFunction(fun a b c d e f g->(c,e,g))
...
//expression.AddProduction(expression, plusplus,expression).SetReduceFunction (fun a _ c -> Ast.BinaryExpression(a, Ast.Add ,c))
